...
from PyQt6.QtWidgets import (
    ...
    QLabel, QGroupBox, QGridLayout, QHeaderView, QFrame
)
...
class StatisticsTableWidget(QWidget):
    ...
    def update_statistics(self, 
        ...
        # Ορισμός στηλών
        headers = ['Κανάλι', 'Αρχικό RMS (μV)', 'Καθαρό RMS (μV)', 
                  'Μείωση (%)', 'Αρχικό Εύρος', 'Καθαρό Εύρος']
            ...
            for col, data in enumerate(row_data):
                ...
                if col == 3:  # Στήλη μείωσης
                    if reduction > 50:
                        ...
                        item.setBackground(QColor("#fff3cd"))  # Κίτρινο για μέτρια μείωση
                    else:
                        item.setBackground(QColor("#f8d7da"))  # Κόκκινο για χαμηλή μείωση
    ...
    def plot_comparison(self, 
        ...
        try:
            ...
            for i, channel in enumerate(channels):
                ...
                # Plots
                ax.plot(display_times, orig_display, 
                       color='#e74c3c', alpha=0.7, linewidth=1.5, 
                       label='Αρχικό σήμα')
                ax.plot(display_times, clean_display, 
                       color='#27ae60', alpha=0.8, linewidth=1.5, 
                       label='Καθαρό σήμα')
                
                # Στυλ
                ax.set_title(f'Κανάλι {channel}', fontsize=10, color='#2c3e50', fontweight='bold')
                ax.set_xlabel('Χρόνος (s)' if i == n_channels - 1 else '', fontsize=9, color='#2c3e50')
            ...
            ax.text(0.5, 0.5, f'Σφάλμα οπτικοποίησης: {str(e)}', 
...
class ResultsDisplayWidget(QWidget):
    ...
    def create_summary_widget(self) -> QWidget:
        ...
        labels = [self.file_label, self.components_label, 
                 self.avg_reduction_label, self.status_label]
    ...
    def update_results(self, 
        ...
        try:
            ...
            self.components_label.setText(f"Συνιστώσες που αφαιρέθηκαν: {components_removed}")
            self.avg_reduction_label.setText(f"Μέση μείωση θορύβου: {avg_reduction:.1f}%")
