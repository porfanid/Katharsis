...
def test_electrode_detection():
    ...
    for i, channels in enumerate(test_cases):
        ...
        try:
            ...
            assert len(detected) == len(channels), f"Expected {len(channels)}, got {len(detected)}"
...
def test_full_pipeline():
    ...
    for channels, description in test_configs:
        ...
        try:
            ...
            assert result['success'], f"Loading failed: {result.get('error', 'Unknown error')}"
            ...
            assert ica_result['success'], f"ICA failed: {ica_result.get('error', 'Unknown error')}"
            assert ica_result['n_components'] == len(channels), f"Component count mismatch"
            print(f"  ✓ ICA successful: {ica_result['n_components']} components")
            
            # Test artifact detection
            detect_result = service.detect_artifacts()
            assert detect_result['success'], f"Detection failed: {detect_result.get('error', 'Unknown error')}"
            print(f"  ✓ Detection successful: {len(detect_result['suggested_artifacts'])} artifacts found")
            
            # Test artifact removal (if artifacts found)
            if len(detect_result['suggested_artifacts']) > 0:
                clean_result = service.apply_artifact_removal([detect_result['suggested_artifacts'][0]])
                assert clean_result['success'], f"Cleaning failed: {clean_result.get('error', 'Unknown error')}"
...
def test_edge_cases():
    ...
    info = mne.create_info(ch_names=all_channels, sfreq=sfreq, ch_types=['eeg']*len(all_channels))
    raw = mne.io.RawArray(data, info)
