...
from backend import (
    EEGDataManager, EEGPreprocessor, EEGBackendCore,
    ICAProcessor, ArtifactDetector, EEGArtifactCleaningService
)
...
class TestEEGDataManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Export to EDF
        self.test_raw.export(self.temp_edf_path, fmt='edf', overwrite=True, verbose=False)
    ...
    def test_save_cleaned_data(self):
        ...
        try:
            success = self.data_manager.save_cleaned_data(self.test_raw, temp_output_path)
...
class TestICAProcessor(unittest.TestCase):
    ...
    def test_get_component_info(self):
        ...
        expected_keys = ['variance', 'kurtosis', 'range', 'std', 'mean', 'rms', 'skewness']
        for key in expected_keys:
...
class TestArtifactDetector(unittest.TestCase):
    ...
    def setUp(self):
        ...
        source1 = np.sin(2 * np.pi * 2 * time) + 0.5 * np.random.randn(n_samples)    # Low freq + noise
        source2 = np.sin(2 * np.pi * 60 * time) + 2.0 * np.random.randn(n_samples)   # High freq + high noise (artifact)
        source3 = np.sin(2 * np.pi * 10 * time) + 0.1 * np.random.randn(n_samples)   # Clean brain signal
...
class TestEEGArtifactCleaningService(unittest.TestCase):
    ...
    def test_full_processing_pipeline(self):
        ...
        # Apply cleaning
        clean_result = self.service.apply_artifact_removal([0])  # Remove first component
    ...
    def test_get_processing_summary(self):
        ...
        expected_keys = ['current_file', 'is_processing', 'ica_fitted', 
                        'n_components', 'suggested_artifacts', 'detection_methods']
